...
const SOCIAL = [
  {
    ...
    label: 'Dierre Impianti su Instagram',
    Icon: InstagramIcon,
    hoverBg: 'linear-gradient(135deg, #f09433 0%, #e6683c 25%, #dc2743 50%, #cc2366 75%, #bc1888 100%)',
    hoverShadow: '0 0 20px rgba(220,39,67,0.45), 0 0 40px rgba(188,24,136,0.2)',
  },
  {
    href: 'https://www.facebook.com/profile.php?id=100042407208094',
    label: 'Dierre Impianti su Facebook',
    Icon: FacebookIcon,
    hoverBg: '#1877F2',
    hoverShadow: '0 0 20px rgba(24,119,242,0.5), 0 0 40px rgba(24,119,242,0.2)',
  },
]
...
export default function Footer() {
  ...
  const goTo      = to => { navigate(to);         window.scrollTo({ top: 0 }) }
  const scrollTop = () => window.scrollTo({ top: 0, behavior: 'smooth' })

  return (
    <footer className="relative bg-dark border-t border-white/6 overflow-hidden" role="contentinfo" aria-label="Piè di pagina">

      {/* Radial glow top-center */}
      <div aria-hidden="true" className="absolute inset-x-0 top-0 h-px"
        style={{ background: 'linear-gradient(90deg, transparent 0%, rgba(56,189,248,0.35) 50%, transparent 100%)' }} />
      <div aria-hidden="true" className="pointer-events-none absolute inset-0"
        style={{ background: 'radial-gradient(ellipse 70% 35% at 50% 0%, rgba(56,189,248,0.05) 0%, transparent 60%)' }} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 lg:gap-20 pt-16 pb-14 border-b border-white/6">
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              {SOCIAL.map(({ href, label, Icon, hoverBg, hoverShadow }) => (
                <motion.a
                  key={href}
                  href={href}
                  target="_blank"
          ...
          <div>
            ...
            <ul className="space-y-3 mb-10" role="list">
              {NAV_LINKS.map(({ label, to }) => (
                <li key={to}>
                  <button
                    onClick={() => goTo(to)}
                    className="group text-text-s hover:text-accent text-sm transition-colors duration-150 text-left inline-flex items-center gap-2"
                  >
                    <span
                      className="inline-block w-3 h-px transition-all duration-200 group-hover:w-5"
                      style={{ background: 'linear-gradient(90deg, #f5c430, #38bdf8)' }}
                      aria-hidden="true"
                    />
                    {label}
                  </button>
            ...
            <ul className="space-y-2.5" role="list">
              {LEGAL_LINKS.map(([l, to]) => (
                <li key={to}>
                  <Link
                    to={to}
                    className="group text-text-s hover:text-accent text-sm transition-colors duration-150 inline-flex items-center gap-2"
                  >
                    <span
                      className="inline-block w-3 h-px transition-all duration-200 group-hover:w-5"
                      style={{ background: 'linear-gradient(90deg, #f5c430, #38bdf8)' }}
          ...
          <div>
            ...
            <a
              ...
              className="group flex items-center gap-3.5 mb-6 p-4 rounded-xl transition-all duration-200 hover:scale-[1.02] active:scale-100"
            ...
            >
              ...
              <div>
                ...
                <p className="font-display font-bold text-text-p text-base group-hover:text-accent transition-colors leading-tight">
                  +39 347 317 7613
                </p>
              </div>
            </a>
        ...
        <div className="py-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <motion.button
              ...
              className="w-8 h-8 rounded-lg flex items-center justify-center text-text-xs hover:text-accent transition-colors duration-200"
